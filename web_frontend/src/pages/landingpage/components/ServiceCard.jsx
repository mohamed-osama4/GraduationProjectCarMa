...
export default function ServiceCard({ icon, title, description, bgColor = "bg-blue-50", price, delay = 0 }) {
  return (
    <div
      className="service-card group flex flex-col h-full bg-white/5 backdrop-blur-md p-6 rounded-[1.5rem] border border-white/10 transition-all duration-500 hover:bg-white/10 hover:border-premium-gold/30 hover:shadow-[0_15px_40px_rgba(0,0,0,0.4)]"
      data-aos="fade-up"
      data-aos-delay={delay}
    >
      <div className="w-16 h-16 mb-6 rounded-xl bg-midnight/60 backdrop-blur-xl border border-white/10 flex items-center justify-center text-premium-gold group-hover:bg-premium-gold group-hover:text-midnight group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(255,195,0,0.4)] transition-all duration-500 relative z-10">
        {React.cloneElement(icon, { size: 32 })}
      </div>
      
      <h3 className="text-xl font-black text-white mb-2 group-hover:text-premium-gold transition-colors duration-300">{title}</h3>
      ...
      {price && (
        <div className="flex justify-between items-end mt-8 pt-6 border-t border-white/10 w-full group-hover:border-premium-gold/20 transition-colors">
          <div>
            <p className="text-[10px] text-silver/40 font-black mb-0.5">يبدأ من</p>
            <p className="font-black text-xl text-premium-gold">{price} <span className="text-xs font-bold opacity-60">جنيه</span></p>
          </div>
